...
bool GenerateDesktopFile(
	...
	if (!Core::UpdaterDisabled()) {
		...
		QFile::remove(u"%1telegramdesktopvless.desktop"_q.arg(targetPath));
		QFile::remove(u"%1unofficial-telegram-desktop-vless.desktop"_q.arg(targetPath));
		...
		if (!Core::Launcher::Instance().customWorkingDir()) {
			QFile::remove(u"%1app.unofficial.telegram.desktop.vless._%2.desktop"_q.arg(
				targetPath,
				md5Hash));
		...
		QFile::remove(u"%1app.unofficial.telegram.desktop.vless.%2.desktop"_q.arg(
			targetPath,
			md5Hash));
	}
...
bool GenerateServiceFile(bool silent = false) {
	...
	if (!Core::UpdaterDisabled()
		...
		QFile::remove(u"%1app.unofficial.telegram.desktop.vless._%2.service"_q.arg(
			targetPath,
			md5Hash));
	}
...
QString psAppDataPath() {
	// Previously we used ~/.TelegramDesktopVless, so look there first.
	// If we find data there, we should still use it.
	auto home = QDir::homePath();
	if (!home.isEmpty()) {
		auto oldPath = home + u"/.TelegramDesktopVless/"_q;
...
void start() {
	QGuiApplication::setDesktopFileName([&] {
		...
		if (!Core::UpdaterDisabled()) {
			return u"app.unofficial.telegram.desktop.vless._%1"_q.arg(
				Core::Launcher::Instance().instanceHash().constData());
		}

		return u"app.unofficial.telegram.desktop.vless"_q;
	}());
