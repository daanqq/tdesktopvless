...
#ifdef OS_WIN_STORE
const WCHAR AppUserModelIdBase[] = L"Unofficial.TelegramDesktopVless.Store";
#else // OS_WIN_STORE
const WCHAR AppUserModelIdBase[] = L"Unofficial.TelegramDesktopVless";
#endif // OS_WIN_STORE
...
void CleanupShortcut() {
	...
	QString path = systemShortcutPath() + u"TelegramDesktopVless.lnk"_q;
...
bool checkInstalled(QString path = {}) {
	...
	const auto installed = u"Unofficial Telegram Desktop VLESS/Unofficial Telegram Desktop VLESS.lnk"_q;
	const auto old = u"TelegramDesktopVless.lnk"_q;
...
bool ValidateShortcut() {
	...
	if (cAlphaVersion()) {
		...
		path += u"TelegramDesktopVless.lnk"_q;
		if (validateShortcutAt(path)) {
			return true;
		}
	}
